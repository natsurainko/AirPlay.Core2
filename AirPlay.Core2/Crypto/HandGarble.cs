...
public static class HandGarble
{
    public static void Garble(byte[] buffer0, byte[] buffer1, byte[] buffer2, byte[] buffer3, byte[] buffer4)
    {
        int tmp, tmp2, tmp3;
        ...
        buffer1[140] += (byte)WeirdRor8(92, buffer1[5] & 0xff & 7);
    ...
    private static int Rol8x(int input, int count) => (input << count) | (input >> (8 - count));

    private static int WeirdRor8(int input, int count)
    {
        if (count == 0) return 0;
        return input >> count & 0xff | (input & 0xff) << 8 - count;
    }

    private static int WeirdRol8(int input, int count)
    {
        if (count == 0) return 0;
        return input << count & 0xff | (input & 0xff) >> 8 - count;
    }

    private static int WeirdRol32(int input, int count)
    {
        if (count == 0) return 0;
