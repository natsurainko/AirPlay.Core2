...
public class OmgHax
{
    public static void DecryptAesKey(byte[] message3, byte[] cipherText, byte[] keyOut)
    {
        var chunk1 = ByteArrayExtensions.CopyOfRange(cipherText, 16, cipherText.Length);
        var chunk2 = ByteArrayExtensions.CopyOfRange(cipherText, 56, cipherText.Length);
    ...
    private static void DecryptMessage(byte[] messageIn, byte[] decryptedMessage)
    {
        ...
        for (int i = 0; i < 8; i++)
        {
            ...
            if (mode == 2 || mode == 1 || mode == 0)
            {
                if (i > 0)
                {
                    XorBlocks(buffer, ByteArrayExtensions.CopyOfRange(messageIn, 0x10 * i, 0x10 * i + 16), xorResult);
            ...
            else
            {
                if (i < 7)
                {
                    XorBlocks(buffer, ByteArrayExtensions.CopyOfRange(messageIn, 0x70 - 0x10 * i, 0x70 - 0x10 * i + 16), xorResult);
    ...
    private static void GenerateKeySchedule(byte[] key_material, int[][] key_schedule)
    {
        var key_data = new int[4];
        var deadbeef = 0xdeadbeef;

        for (int i = 0; i < 11; i++)
            key_schedule[i] = [(byte)deadbeef, (byte)deadbeef, (byte)deadbeef, (byte)deadbeef];
    ...
    private static void GenerateSessionKey(byte[] oldSap, byte[] messageIn, byte[] sessionKey)
    {
        ...
        {
            var @base = ByteArrayExtensions.CopyOfRange(newSap, round * 64, newSap.Length);
            ModifiedMD5.ModifiedMd5(@base, sessionKey, md5);
            SapHash.Hash(@base, sessionKey);
        ...
        {
            (sessionKey[i + 3], sessionKey[i]) = (sessionKey[i], sessionKey[i + 3]);
            (sessionKey[i + 2], sessionKey[i + 1]) = (sessionKey[i + 1], sessionKey[i + 2]);
        }
    ...
    private static void Cycle(byte[] block, int[][] key_schedule)
    {
    ...
    private static void XorBlocks(byte[] a, byte[] b, byte[] @out)
    {
        for (int i = 0; i < 16; i++)
            @out[i] = (byte)(a[i] ^ b[i]);
    }

    private static void ZXor(byte[] @in, byte[] @out, int blocks)
    {
    ...
    private static void XXor(byte[] @in, byte[] @out, int blocks)
    {
        for (int j = 0; j < blocks; j++)
            for (int i = 0; i < 16; i++)
    ...
    private static void TXor(byte[] @in, byte[] @out)
    {
        for (int i = 0; i < 16; i++)
            @out[i] = (byte)(@in[i] ^ OmgHaxHex.TKey[i]);
    }

    private static byte[] TableIndex(int i) => ByteArrayExtensions.CopyOfRange(OmgHaxHex.TableS1, 31 * i % 0x28 << 8, OmgHaxHex.TableS1.Length);

    private static byte[] MessageTableIndex(int i) => ByteArrayExtensions.CopyOfRange(OmgHaxHex.TableS2, 97 * i % 144 << 8, OmgHaxHex.TableS2.Length);
    ...
    private static byte[] PermuteTable2(int i) => ByteArrayExtensions.CopyOfRange(OmgHaxHex.TableS4, 71 * i % 144 << 8, OmgHaxHex.TableS4.Length);
