using System.Reflection;

namespace AirPlay.Core2.Crypto;

public static class OmgHaxHex
{
    public readonly static byte[] StaticSource1 = [0xFA, 0x9C, 0xAD, 0x4D, 0x4B, 0x68, 0x26, 0x8C, 0x7F, 0xF3, 0x88, 0x99, 0xDE, 0x92, 0x2E, 0x95, 0x1E];
    public readonly static byte[] StaticSource2 = [0xEC, 0x4E, 0x27, 0x5E, 0xFD, 0xF2, 0xE8, 0x30, 0x97, 0xAE, 0x70, 0xFB, 0xE0, 0x00, 0x3F, 0x1C, 0x39, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x10, 0x09, 0x00, 0x0, 0x00, 0x00, 0x00, 0x00];
    public readonly static byte[] InitialSessionKey = [0xDC, 0xDC, 0xF3, 0xB9, 0x0B, 0x74, 0xDC, 0xFB, 0x86, 0x7F, 0xF7, 0x60, 0x16, 0x72, 0x90, 0x51];
    public readonly static byte[] DefaultSap =
    [
        
    ];
    public static readonly byte[] IndexMAngle = [0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1B, 0x36, 0x6C];

    public static readonly byte[][] MessageKey = 
    [
        [],
        [],
        [],
        []
    ];

    public static readonly byte[][] MessageIv = 
    [
        [0x57, 0x52, 0xF1, 0xB7, 0x54, 0x9D, 0x8F, 0x87, 0x0C, 0x10, 0x48, 0x5A, 0x60, 0x88, 0xCA, 0xDB],
        [0xDF, 0x7B, 0x15, 0x63, 0xF0, 0x05, 0x58, 0x77, 0x52, 0xA9, 0x04, 0x02, 0xB9, 0xA3, 0x92, 0x95],
        [0x68, 0xB5, 0x46, 0x11, 0xFB, 0x04, 0xDE, 0x67, 0x6C, 0x96, 0x8E, 0xFB, 0x8C, 0x9D, 0xB0, 0xC9],
        [0x27, 0x07, 0x8B, 0x21, 0x23, 0x36, 0x1E, 0x7A, 0xDC, 0x9D, 0x0B, 0x11, 0x53, 0x54, 0x69, 0x0D]
    ];

    public static readonly byte[] TKey = [0xd0, 0x04, 0xa9, 0x61, 0x6b, 0xa4, 0x00, 0x87, 0x68, 0x8b, 0x5f, 0x15, 0x15, 0x35, 0xd9, 0xa9];
    public static readonly byte[] XKey = [0x8e, 0xba, 0x07, 0xcc, 0xb6, 0x5a, 0xf6, 0x20, 0x33, 0xcf, 0xf8, 0x42, 0xe5, 0xd5, 0x5a, 0x7d];
    public static readonly byte[] ZKey = [0x1a, 0x64, 0xf9, 0x60, 0x6c, 0xe3, 0x01, 0xa9, 0x54, 0x48, 0x1b, 0xd4, 0xab, 0x81, 0xfc, 0xc6];

    public static readonly byte[] TableS1;
    public static readonly byte[] TableS2;
    public static readonly byte[] TableS3;
    public static readonly byte[] TableS4;
    public static readonly int[] TableS5;
    public static readonly int[] TableS6;
    public static readonly int[] TableS7;
    public static readonly int[] TableS8;
    public static readonly int[] TableS9;
    public static readonly byte[] TableS10;

    static OmgHaxHex()
    {
        TableS1 = ReadTableBytes("table_s1");
        TableS2 = ReadTableBytes("table_s2");
        TableS3 = ReadTableBytes("table_s3");
        TableS4 = ReadTableBytes("table_s4");
        TableS5 = ReadTableInts("table_s5");
        TableS6 = ReadTableInts("table_s6");
        TableS7 = ReadTableInts("table_s7");
        TableS8 = ReadTableInts("table_s8");
        TableS9 = ReadTableInts("table_s9");
        TableS10 = ReadTableBytes("table_s10");
    }

    private static byte[] ReadTableBytes(string key)
    {
        Assembly assembly = Assembly.GetExecutingAssembly();
        using var stream = assembly.GetManifestResourceStream($"AirPlay.Core2.Resources.{key}.bin") ?? throw new IOException();
        using var reader = new StreamReader(stream);

        string data = reader.ReadToEnd();
        string[] hexs = [.. data.Split(',', StringSplitOptions.RemoveEmptyEntries).Select(h => h.Replace("0x", string.Empty).Trim())];

        byte[] result = new byte[hexs.Length];
        for (int i = 0; i < hexs.Length; i++)
            result[i] = Convert.ToByte(hexs[i], 16);

        return result;
    }

    private static int[] ReadTableInts(string key)
    {
        Assembly assembly = Assembly.GetExecutingAssembly();
        using var stream = assembly.GetManifestResourceStream($"AirPlay.Core2.Resources.{key}.bin") ?? throw new IOException();
        using var reader = new StreamReader(stream);

        string data = reader.ReadToEnd();
        string[] hexs = [.. data.Split(',', StringSplitOptions.RemoveEmptyEntries).Select(h => h.Replace("0x", string.Empty).Trim())];

        int[] result = new int[hexs.Length];
        for (int i = 0; i < hexs.Length; i++)
            result[i] = Convert.ToInt32(hexs[i], 16);

        return result;
    }
}