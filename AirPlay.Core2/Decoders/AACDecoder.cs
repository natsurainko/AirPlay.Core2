...
public class AACDecoder : IDecoder//, IDisposable
{
    ...
    public int Config(int sampleRate, int channels, int bitDepth, int frameLength)
    {
        ...
        else throw new NotSupportedException($"Channel count {channels} is not supported.");

        _jaadDecoder = new Decoder(_decoderConfig);
        return 0;
    }

    public int GetOutputStreamLength() => _pcmPktSize;
...
public struct CStreamInfo
{
    ...
    nint pChannelType; /*!< Audio channel type of each output audio channel. */

    nint pChannelIndices; /*c!< Audio channel index for each output audio
